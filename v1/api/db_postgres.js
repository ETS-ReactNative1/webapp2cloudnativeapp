...
var PG = require( 'pg' );
var database_url = 'POSTGRES_DATABASE_URL' in process.env ? process.env.POSTGRES_DATABASE_URL : 'postgres://user:pass@localhost:5432/db'; 
...
api.createItem = async function( item ){
  return new Promise( async ( resolve, reject ) => {
    if( pg ){
      ...
      if( conn ){
        try{
          var sql = 'insert into items( id, name, price, username, created, updated ) values ( $1, $2, $3, $4, $5, $6 )';
          ...
          item.updated = t;
          //console.log( item );
          var query = { text: sql, values: [ item.id, item.name, item.price, item.username, item.created, item.updated ] };
    ...
    if( pg ){
      ...
      if( conn ){
        try{
          ...
          var sql = 'insert into items( id, name, price, username, created, updated ) values ( $1, $2, $3, $4, $5i, $6 )';
...
api.queryItems = async function( key, limit, offset ){
  return new Promise( async ( resolve, reject ) => {
    if( pg ){
      ...
      if( conn ){
        try{
          var sql = "select * from items where name like '%" + key + "%' or username like '%" + key + "%' order by updated";
          if( limit ){
            sql += " limit " + limit;
          }
          if( offset ){
    ...
    if( pg ){
      ...
      if( conn ){
        if( !item.id ){
          ...
          try{
            var sql = 'update items set name = $1, price = $2, username = $3, updated = $4 where id = $5';
            //var sql = "select * from items";
            var t = ( new Date() ).getTime();
            item.updated = t;
            var query = { text: sql, values: [ item.name, item.price, item.username, item.updated, item.id ] };
            conn.query( query, function( err, result ){
              if( err ){
                console.log( err );
                resolve( { status: false, error: err } );
